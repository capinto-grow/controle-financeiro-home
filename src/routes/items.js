...
router.post('/maps/:mapId/items', async (req, res) => {
  try {
    ...
    const monthArr = Array.isArray(months) && months.length === 12
      ? months.map(v => parseFloat(v)||0) : Array(12).fill(0);
    const id = uuidv4();
    const result = await db.query(
      `INSERT INTO map_items (id, map_id, type, description, category, due_day, months, display_order)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [id, req.params.mapId, type, description, category||null, dueDay||null, monthArr, displayOrder||0]
    );
...
router.put('/items/:id', async (req, res) => {
  try {
    ...
    const result = await db.query(
      `UPDATE map_items SET description=COALESCE($1,description), category=COALESCE($2,category),
       due_day=$3, months=COALESCE($4,months), display_order=COALESCE($5,display_order), updated_at=NOW()
...
router.patch('/items/:id/month', async (req, res) => {
  try {
    ...
    const result = await db.query(
      `UPDATE map_items SET months[$1]=$2, updated_at=NOW() WHERE id=$3 RETURNING *`,
      [monthIndex + 1, parseFloat(value)||0, req.params.id]
    );
    if (!result.rows[0]) return res.status(404).json({ error: 'Item não encontrado' });
    res.json(result.rows[0]);
  } catch (err) { res.status(500).json({ error: 'Erro ao atualizar mês' }); }
});
