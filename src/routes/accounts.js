...
router.get('/', async (req, res) => {
  try {
    const result = await db.query(
      'SELECT * FROM financial_accounts WHERE workspace_id=$1 ORDER BY name',
...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO financial_accounts (workspace_id, name, type, institution, opening_balance, current_balance, notes)
       VALUES ($1,$2,$3,$4,$5,$5,$6) RETURNING *`,
      [req.user.workspaceId, name, type || 'conta_corrente', institution, parseFloat(openingBalance)||0, notes]
    );
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE financial_accounts SET name=COALESCE($1,name), institution=COALESCE($2,institution),
       current_balance=COALESCE($3,current_balance), is_active=COALESCE($4,is_active), notes=COALESCE($5,notes), updated_at=NOW()
